...
当前资源流程会将其应用到 Android 和 iOS。生成的 HarmonyOS Shell 使用
`harmony/AppScope/resources` 下独立维护的图标资源。
...
```ts
optional harmonyAssets: string;
```

相对于项目根目录的 HarmonyOS rawfile 目录路径。

#### Default Value

`{ androidAssets: 'android/app/src/main/assets', iosAssets: 'ios/LynxResources/Assets', harmonyAssets: 'harmony/entry/src/main/resources/rawfile' }`
...
Android、iOS 和 HarmonyOS 构建的平台标识配置。
