...
:::note HarmonyOS 范围
当前 HarmonyOS 支持由 `create-sparkling-app` 生成的 ArkTS Shell 提供，尚未发布可独立复用的
HarmonyOS Sparkling 容器 SDK。已有 HarmonyOS 应用应以生成项目中的 `harmony/` 为宿主参考，
不能直接套用下文 Android/iOS 的依赖接入步骤。
:::
...
`SPKExecuteAllPrepareBootTask()` 已经移动到 `SparklingMacro` 中，因此你的 App Target
需要额外添加 `SparklingMacro` pod，并在调用前先 `import SparklingMacro`。
