...
Host 决定使用哪种原生容器来渲染内容。参见[容器](./containers)了解各容器模式的详情。
...
通常不需要手动构建 scheme URL。[`navigate()`](/apis/sparkling-methods/sparkling-navigation) 函数会为你构建它们。参见[多页面导航](./multi-page-navigation)了解常见的导航模式（传递数据、关闭页面等）。
