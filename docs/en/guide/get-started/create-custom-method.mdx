...
A Sparkling Method is a typed JS ↔ native bridge package for Android, iOS, and HarmonyOS. You define the API in TypeScript, generate Kotlin and Swift code with the CLI, and implement the generated HarmonyOS handler in ArkTS.

:::note HarmonyOS scope
The method code generator currently produces Kotlin and Swift implementations only. A method
package can nevertheless ship a handwritten ArkTS handler: HarmonyOS autolink copies that source
into the app, generates a static import, and dispatches its declared method names through `spkPipe`.
:::
...
└── harmony/
    └── *HarmonyHandler.ets   # HarmonyOS handler scaffold
```
...
The scaffold includes the following HarmonyOS metadata. Codegen keeps `methodNames` synchronized
with the TypeScript declarations:

```json
{
  "harmony": {
    "methodNames": ["myGreeting.hello"],
    "sourceDir": "harmony",
    "entry": "MyGreetingHandler.ets",
    "className": "MyGreetingHandler"
  }
}
```

The exported class must provide `call(context, method, request, callback)`. The callback ABI is
`callback(code: number, msg: string, data?: Object)`. For example:

```ts
type MethodCallback = (code: number, msg: string, data?: Object) => void;

export class MyGreetingHandler {
  call(context: Context, method: string, request: Object, callback: MethodCallback): void {
    callback(1, 'ok');
  }
}
```
...
Then run autolink to refresh the Sparkling method registry:
...
- **Android**: links the Sparkling method Gradle project and generates the `SparklingAutolink.kt` registry.
- **iOS**: links the Sparkling method pod and generates the `SparklingAutolink.swift` registry.
- **HarmonyOS**: copies a declared ArkTS source tree and generates imports and dispatch code in `SparklingAutolink.ets`.
...
- **Platform parity**: implement the same behavior on Android, iOS, and HarmonyOS.
- **Error handling**: return `code: 1` for success, `code: 0` for failure, and negative codes for errors,
  with a descriptive `msg`. See [Response codes](/apis/response-codes).
- **Testing**: test on all supported platforms before publishing.
