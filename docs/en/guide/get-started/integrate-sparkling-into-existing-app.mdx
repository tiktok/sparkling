...
`SPKExecuteAllPrepareBootTask()` now lives in `SparklingMacro`, so your app target
must add the `SparklingMacro` pod and import it before calling this function.
...
```
  ...
  Embedded containers require manual lifecycle forwarding — see [Containers](../containers) for the full guide.
...
- **Blank screen / failed to load**: verify the `bundle=` name exists in app assets and your scheme
  matches the host type (`lynxview_page`, `lynxview_card`, etc.). See [Scheme](../../apis/scheme) for details.
