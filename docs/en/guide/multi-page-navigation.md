...
Sparkling uses a native [container](./containers) model where each page runs as an independent native container with its own LynxView. Navigation between pages is driven by [scheme URLs](./scheme).
...
1. Builds a [scheme URL](./scheme) (e.g. `hybrid://lynxview_page?bundle=detail.lynx.bundle&title=Detail`)
2. Hands the URL to the native layer, which opens a new [container](./containers)
...
The `params` object supports all [predefined scheme parameters](./scheme#common-parameters) (`title`, `hide_nav_bar`, `screen_orientation`, etc.) as well as arbitrary custom keys.
