...
machine's LAN IP address. `sparkling run:android` uses that LAN IP for physical
devices. If automatic detection picks the wrong network, pass `--host <host>` or
set `dev.server.host` in `app.config.ts`.
