...
`sparkling run:android` binds the dev server to that specific LAN IP for physical
devices. It does not bind to `0.0.0.0` by default, because that would expose the
dev server on every network interface. If automatic detection picks the wrong
network, pass `--host <host>` or set `dev.server.host` in `app.config.ts`.
