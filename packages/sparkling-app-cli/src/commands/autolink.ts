...
async function discoverModules(cwd: string): Promise<MethodModuleConfig[]> {
  ...
  for (const search of searches) {
    ...
    for (const configPath of matches) {
      ...
      const nodeApiAddons = Array.isArray(config.nodeApiAddons)
        ? (config.nodeApiAddons as unknown[]).filter(
          (entry): entry is string => typeof entry === 'string' && /^[A-Za-z0-9_]+$/.test(entry),
        )
        : undefined;
      ...
      seen.set(name, {
        ...
        nodeApiAddons: nodeApiAddons && nodeApiAddons.length > 0 ? nodeApiAddons : undefined,
        android: {
...
/**
 * PrimJS's Node-API implementation, and the adapter Lynx reaches it through.
 *
 * These have to be in the process *before* Lynx initialises: Lynx decides
 * whether to attach a Node-API environment while it starts up, and only does so
 * if it can find them. They ship inside the PrimJS AAR and nothing loads them on
 * its own.
 */
const NODE_API_RUNTIME_LIBRARIES = ['napi', 'napi_adapter'];

/** The libraries an app has to load, in the order they have to be loaded in. */
function nodeApiLibraries(modules: MethodModuleConfig[]): string[] {
  const addons = modules.flatMap(m => m.nodeApiAddons ?? []);
  if (addons.length === 0) {
    return [];
  }
  return [...NODE_API_RUNTIME_LIBRARIES, ...Array.from(new Set(addons))];
}
...
function writeAndroidRegistry(modules: MethodModuleConfig[], appPackage: string, cwd: string) {
  ...
  const libraries = nodeApiLibraries(modules);
  const nodeApiBlock = libraries.length
    ? [
      '',
      '    /**',
      '     * Node-API libraries, in the order they have to be loaded in.',
      '     *',
      '     * The two runtime libraries come first because Lynx decides whether to',
      '     * attach a Node-API environment while it initialises, and only does so if',
      '     * they are already in the process. Each addon is loaded after them, which',
      '     * runs its napi_module_register constructor and makes it resolvable from',
      '     * the page through getNapiLoader().load(name).',
      '     */',
      '    val nodeApiLibraries =',
      '        listOf(',
      ...libraries.map(name => `            "${name}",`),
      '        )',
      '',
      '    /**',
      '     * Load them. Call from Application.onCreate, after Lynx is initialised and',
      '     * before any LynxView is created. A library that is not there is logged and',
      '     * skipped rather than thrown, so a build without the addon still runs.',
      '     */',
      '    @JvmStatic',
      '    fun loadNodeApiAddons() {',
      '        for (library in nodeApiLibraries) {',
      '            runCatching { System.loadLibrary(library) }',
      '                .onFailure { error ->',
      '                    android.util.Log.w("SparklingAutolink", "lib$library.so not loaded", error)',
      '                }',
      '        }',
      '    }',
    ]
...
function writeIosRegistry(modules: MethodModuleConfig[], bundleId: string, cwd: string) {
  ...
  const addonList = Array.from(new Set(modules.flatMap(m => m.nodeApiAddons ?? [])))
    .map(name => `"${name}"`)
    .join(', ');
  ...
  const content = [
    ...
    ']',
    '',
    '/// Node-API addons linked into this app.',
    '///',
    '/// Registration itself is a C macro - `NAPI_USE(<Addon>)` in a compiled',
    '/// translation unit, from the addon\'s own `addon_use.h` - so it cannot be',
    '/// generated from Swift. This list is what the app has to name there, and',
    '/// requires the `LynxWeakNodeAPI` pod to be in the Podfile.',
    `let sparklingAutolinkNodeApiAddons: [String] = [${addonList}]`,
    '',
  ].join('\n');
