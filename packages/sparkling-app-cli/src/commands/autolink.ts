...
async function discoverModules(cwd: string): Promise<MethodModuleConfig[]> {
  ...
  for (const search of searches) {
    ...
    for (const configPath of matches) {
      ...
      const webConfig = config.web as Record<string, unknown> | undefined;
      const platforms = Array.isArray(config.platforms) ? config.platforms as string[] : undefined;
      ...
      seen.set(name, {
        ...
        web: webConfig ? {
          entryPoint: typeof webConfig.entryPoint === 'string' ? webConfig.entryPoint : undefined,
          subpath: typeof webConfig.subpath === 'string' ? webConfig.subpath : './web',
        } : undefined,
      });
    }
  }
...
function writeWebAutolink(modules: MethodModuleConfig[], cwd: string) {
  const tempDir = path.resolve(cwd, '.sparkling');
  fs.ensureDirSync(tempDir);
  const filePath = path.join(tempDir, 'web-autolink.ts');

  const webModules = modules.filter(m =>
    (m.platforms && m.platforms.includes('web')) || m.web,
  );
  const skippedModules = modules.filter(m =>
    !(m.platforms && m.platforms.includes('web')) && !m.web,
  );

  const lines: string[] = [
    '// Generated by sparkling autolink — do not edit',
  ];

  for (const mod of webModules) {
    const subpath = mod.web?.subpath ?? './web';
    const importPath = `${mod.name}/${subpath.replace(/^\.\//, '')}`;
    lines.push(`import '${importPath}';`);
  }

  for (const mod of skippedModules) {
    lines.push(`// ${mod.name}: web platform not supported, skipping`);
  }
  ...
  if (isVerboseEnabled()) {
    verboseLog(`Web autolink: ${webModules.length} module(s) linked, ${skippedModules.length} skipped`);
    verboseLog(`Web autolink written to ${filePath}`);
  }
}
...
export async function autolink(options: AutolinkOptions): Promise<MethodModuleConfig[]> {
  ...
  const doWeb = platform === 'web' || platform === 'all';
  const modules = await discoverModules(options.cwd);
  if (isVerboseEnabled()) {
    const moduleNames = modules.map(m => m.name).join(', ') || '(none)';
    verboseLog(`Autolink platforms -> android: ${doAndroid}, ios: ${doIos}, web: ${doWeb}`);
  ...
  if (doWeb) {
    writeWebAutolink(modules, options.cwd);
  }

  const platformLabels: string[] = [];
  if (doAndroid) platformLabels.push('Android');
  if (doIos) platformLabels.push('iOS');
  if (doWeb) platformLabels.push('Web');
  const platformLabel = platformLabels.join(', ') || platform.toUpperCase();
  if (modules.length) {
