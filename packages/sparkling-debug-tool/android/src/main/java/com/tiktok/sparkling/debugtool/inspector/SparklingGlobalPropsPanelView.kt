 ...
 */
internal class SparklingGlobalPropsPanelView @JvmOverloads constructor(
    context: Context,
    ...
    private fun matches(key: String, value: Any?, needle: String): Boolean {
    ...
    private fun formatValue(value: Any?): String = when (value) {
        null -> "null"
        is Map<*, *> -> try { JSONObject(value).toString(2) } catch (_: Throwable) { value.toString() }
        is List<*> -> try { JSONArray(value).toString(2) } catch (_: Throwable) { value.toString() }
        is Array<*> -> value.toList().toString()
        else -> value.toString()
    }

    private sealed class Row {
        data class Section(val title: String, val subtitle: String) : Row()
        data class Header(val name: String) : Row()
        data class KV(val key: String, val value: String) : Row()
        data class QueryItems(val containerKey: String, val count: Int, val expanded: Boolean) : Row()
    }
    ...
    private class KVHolder(view: View) : RecyclerView.ViewHolder(view) {
