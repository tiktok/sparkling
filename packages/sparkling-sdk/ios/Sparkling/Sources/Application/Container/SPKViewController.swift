    ...
    var topOffset: CGFloat {
        ...
        if config?.transStatusBar == true {
            ...
            if config?.hideStatusBar == false
                || ((UIDevice.spk.isIPhoneXSeries && config?.hideNavBar == false) && CGRectGetWidth(self.view.frame) < CGRectGetHeight(self.view.frame))
            {
