...
struct SPKColorTests {
    ...
    @Test func testColorRGBAInitializer() {
        let rgbaValue: UInt32 = 0x3366_9980
    ...
    @Test func testColorRGBA_alphaExtraction() {
        ...
        let transparentColor = SPKKitWrapper<UIColor>.color(rgba: 0xFF00_0000)
        #expect(round(transparentColor.cgColor.alpha * 255) == 0)

        let opaqueColor = SPKKitWrapper<UIColor>.color(rgba: 0xFF00_00FF)
        #expect(round(opaqueColor.cgColor.alpha * 255) == 255)

        let halfAlphaColor = SPKKitWrapper<UIColor>.color(rgba: 0xFF00_0080)
